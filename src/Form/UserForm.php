...
class UserForm extends Form
{
    ...
    public function init()
    {
        ...
        if(!empty($adusrinf = $this->getSets('addition_user_information'))){
            ...
            foreach($adusrinf as $opname => $oplabel){
                $formFieldset->add([
                    ...
                    'options' => [
                        'label' => $oplabel,
                        'element_group' => 'addition_information',
                    ],
                    'attributes' => [
                        'id' => $opname
                    ]
                ]);
                if(!empty($userId) && !empty($userSettings = $this->getUserSets($opname, $userId))){
                    $formFieldset->get('o:'.$opname)->setValue($userSettings);
                }
            }
        }
        ...
        $default_resource_template = ($userId ? $this->userSettings->get('default_resource_template', null, $userId) : ($allowed_resource_template ? current($allowed_resource_template) : ''));
        if(!empty($this->getCurrentRoleOps('o:hide_default_resource_template'))){
            $settingsFieldset->add([
                'name' => 'default_resource_template',
                'type' => 'hidden',
                'attributes' => [
                    'value' => $default_resource_template,
                    'id' => 'default_resource_template',
                ],
            ]);
        }else{
            $query['sort_by'] = 'label';
            if(!empty($allowed_resource_template)){
                $query['id'] = $allowed_resource_template;
                if(count($allowed_resource_template) == 1){
                    $default_resource_template = current($allowed_resource_template);
                }
            }
            $settingsFieldset->add([
                'name' => 'default_resource_template',
        ...
        $disable_group_by_owner = False;
        if(!empty($this->getCurrentRoleOps('o:withoutowner_item_set_selector')) || $this->getSets('withoutowner_item_set_selector') == 'true'){
            $disable_group_by_owner = True;
        }

        $allowed_item_sets = $this->getCurrentRoleOps('o:allowed_item_sets');
        $default_item_sets = ($userId ? $this->userSettings->get('default_item_sets', null, $userId) : ($allowed_item_sets ? $allowed_item_sets : []));
        ...
        if(!empty($this->getCurrentRoleOps('o:hide_item_sets_select'))){
            $settingsFieldset->add([
                ...
                'type' => 'hidden',
                'attributes' => [
                    'value' => is_array($default_item_sets) ? '["'.join('", "', $default_item_sets).'"]' : $default_item_sets,
                    'id' => 'default_item_sets',
                ],
            ]);
        }else{
            $query['sort_by'] = 'label';
            if(!empty($allowed_item_sets)){
                $query['id'] = $allowed_item_sets;
                if(count($allowed_item_sets) == 1){
                    $default_item_sets = current($allowed_item_sets);
                }
            }
            $settingsFieldset->add([
                'name' => 'default_item_sets',
                ...
                'options' => [
                    ...
                    'empty_option' => '',
                    'disable_group_by_owner' => $disable_group_by_owner,
                    'query' => $query,
                ],
            ]);
        }
        ...
        $disable_group_by_owner = False;
        if(!empty($this->getCurrentRoleOps('o:withoutowner_item_set_selector')) || !empty($this->getSets('withoutowner_item_set_selector'))){
            $disable_group_by_owner = True;
        }

        $allowed_item_sites = $this->getCurrentRoleOps('o:allowed_item_sites');
        $default_item_sites = ($userId ? $this->userSettings->get('default_item_sites', null, $userId) : ($allowed_item_sites ? $allowed_item_sites : []));
        ...
        if(!empty($this->getCurrentRoleOps('o:hide_site_selector'))){
            $settingsFieldset->add([
                ...
                'type' => 'hidden',
                'attributes' => [
                    'value' => is_array($default_item_sites) ? '["'.join('", "', $default_item_sites).'"]' : $default_item_sites,
            ...
            $query['sort_by'] = 'label';
            if(!empty($allowed_item_sites)){
                $filter_resource_representations = Null;
                $query['id'] = $allowed_item_sites;
                if(count($allowed_item_sites) == 1){
                    $default_item_sites = current($allowed_item_sites);
                }
            }
            $settingsFieldset->add([
                'name' => 'default_item_sites',
                ...
                'options' => [
                    ...
                    'query' => $query,
                    'empty_option' => '',
                    'filter_resource_representations' => $filter_resource_representations,
                ],
            ]);
        }

        if(empty($this->getCurrentRoleOps('o:remove_columns_admin_items'))){
            $settingsFieldset->add([
        ...
        if(empty($this->getCurrentRoleOps('o:remove_columns_admin_item_sets'))){
            $settingsFieldset->add([
        ...
        if(empty($this->getCurrentRoleOps('o:remove_columns_admin_media'))){
            $settingsFieldset->add([
        ...
        if(empty($this->getCurrentRoleOps('o:remove_columns_admin_sites'))){
            $settingsFieldset->add([
        ...
        if(empty($this->getCurrentRoleOps('o:remove_browse_defaults_admin_items'))){
            $settingsFieldset->add([
        ...
        if(empty($this->getCurrentRoleOps('o:remove_browse_defaults_admin_item_sets'))){
            $settingsFieldset->add([
        ...
        if(empty($this->getCurrentRoleOps('o:remove_browse_defaults_admin_media'))){
            $settingsFieldset->add([
        ...
        if(empty($this->getCurrentRoleOps('o:remove_browse_defaults_admin_sites'))){
            $settingsFieldset->add([
